...
### 1. Background thread state persistence
...
### 2. `/approve` text command has no stage token
...
### 3. Stage 0 runtime settings can regress if scattered again
...
### 4. web_search cost and timeout behavior
...
### 5. Caption timing may still need empirical tuning
...
### 6. Render progress is based on ffmpeg progress file

Status: implemented.

`2_render.sh` writes `render_progress.txt` via ffmpeg `-progress`. Slack reads it and send checkpoints: start, 25%, 50%, 75%, complete. Very short renders may skip intermediate checkpoints.

### 7. TTS CLI path under systemd
...
### 10. Slack file editing UX limitations
...
### 12. Encoding in Windows terminal output
...
### 13. `hook_open_loop` / title-hashtag are warnings, not gates (new, 2026-08-01)
...
### 14. `classify_api_error` misclassified `RefreshError`, masking OAuth re-auth need for days (fixed 2026-08-01)
...
### 15. `over_target_length` demoted to warning (2026-08-02)
...
### 16. story_type mix resets if `data/work/` is pruned (2026-08-02)
...
### 17. Script-stage guard deliberately not wired for story_type checks (2026-08-02)
...
### 18. X(Twitter) auto-posting assumptions are unverified against a live account (2026-08-02)
...
### 19. Seed anchoring can thin out the autocomplete pool (2026-08-07)
...
### 20. `threshold` is calibrated against the current weights, not independent of them (2026-08-07)
...
- 3.6–8 second retention drop-off on newly published Shorts (see item 13) — target ≤10 percentage points on the first 5 videos made with the new prompt; if 10–20pp, tighten the Scene 2 rule further; if >20pp, the hypothesis is wrong and the cause is probably audio/caption density, not script structure.
