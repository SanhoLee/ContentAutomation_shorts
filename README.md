...
- 어색한 문장이 보이면 `/x_thread`로 초안을 다시 만든 뒤 `/x_post`로 게시하면
  됩니다.
- **어순 경고**: 승인 화면과 `/x_thread` 미리보기에 `안`/`못` 부정 위치 오류가
  있으면 경고를 함께 띄웁니다(`korean_grammar.negation_placement_issues`).
  "일이 **안 손에** 잡힐 땐"처럼 부정 부사가 용언이 아니라 명사구 앞에 붙은
  경우를 잡습니다. 오탐을 줄이려고 명사에만 붙는 조사(`에`, `에서`, `까지` 등)로
  판정 범위를 좁혔기 때문에 **경고가 없다고 문법이 맞다는 뜻은 아닙니다** —
  이 한 가지 오류가 없다는 뜻입니다. 경고는 게시를 막지 않습니다.
