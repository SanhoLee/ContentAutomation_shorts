...
- **X 스레드는 절대 자동으로 게시되지 않습니다.** YouTube 업로드가 끝나면 봇이 스레드 전문과 이미지를 보여주고 `X 스레드 게시 ▶` / `게시하지 않음` 버튼을 띄웁니다. 승인해야만 나갑니다. 어색한 문장이 보이면 `/x_thread`로 초안을 다시 만든 뒤 게시하세요.
- 승인 화면에는 `안`/`못` 어순 오류 경고가 함께 뜹니다("일이 **안 손에** 잡힐 땐" 같은 경우). 경고가 게시를 막지는 않으니 확인 후 판단하시면 되고, 경고가 없다고 문법이 완벽하다는 뜻은 아닙니다.
- 이 승인은 쇼츠 파이프라인과 독립적입니다. 영상은 승인 여부와 무관하게 예정대로 업로드되고, 승인이 늦어 다음 편 제작이 시작된 뒤에 눌러도 그 작업을 방해하지 않습니다. `게시하지 않음`을 고르면 다시 묻지 않지만 초안은 남아 있어 나중에 `/x_post`로 올릴 수 있습니다.
